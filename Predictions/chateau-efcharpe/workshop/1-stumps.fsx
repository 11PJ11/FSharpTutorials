...
let calcCost (wine:Wine) predictor =
    let quality = wine.Quality
    let predicted = predictor wine
    pown (quality - predicted) 2

let calcAvgCost predictor =
    redWines
    |> Seq.take 20
    |> Seq.map (fun wine -> calcCost wine predictor)
    |> Seq.average
...
let cost0 = calcAvgCost veryBasicPredictor
let cost1 = calcAvgCost alcoholStump1
let cost2 = calcAvgCost alcoholStump2
...
let bestStump =
    ...
    |> Seq.minBy (fun stump -> redWines 
                               |> Seq.map stump
                               //find the minimum cost 
                               //not the minimum prediction
                               |> Seq.min) 
