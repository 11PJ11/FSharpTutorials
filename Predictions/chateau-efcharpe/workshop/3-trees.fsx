...
(*
 l'albero costruisce una gerarchia di domande rilevanti per 
 partizionare l'insieme il piu velocemente possibile.
 Ad esempio come nel gioco in cui i bambini fanno delle domande
 per trovare velocemente un personaggio o se e' un animale 
 o no etc...   
*)
...
cost sample (treePredict manualTree)
cost sample (treePredict deeperTree)
...
let rec learnTree 
    ...
    if (depth >= config.MaxDepth // if we reached maximum depth
    || sampleSize <= config.MinLeafSize) // or if we do not have enough data left,
    then 
        let prediction = 
            sample 
...
let sampleSize = sample |> Seq.length
let toTrain = (int)(floor ((float)sampleSize * 0.99))
let toTest = sampleSize - toTrain
let trainingSample = sample |> Seq.take  toTrain //(sampleSize/2)
let testingSample = sample |> Seq.skip   toTest //(sampleSize/2)
...
[ 1 .. 10 ]
|> List.map (fun depth -> //for each edpth
    let config = { validationConfig with MaxDepth = depth }
