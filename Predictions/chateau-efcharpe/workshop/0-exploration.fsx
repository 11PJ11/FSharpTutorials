...
let minQuality = redWines
               |> Seq.map (fun wine -> wine.Quality)
               |> Seq.min

let maxQuality = redWines
               |> Seq.map (fun wine -> wine.Quality)
               |> Seq.max

let sampleSize = Seq.length redWines
...
let showFor (wines:Wine seq) (selector:Wine -> 'a * 'b) :unit =
    wines
    |> Seq.map selector
    |> Chart.Scatter
    |> Chart.WithOptions options
...
showFor redWines (fun wine -> wine.Alcohol, wine.Quality)
showFor redWines (fun wine -> wine.Chlorides, wine.Quality)
showFor redWines (fun wine -> wine.``Citric acid``, wine.Quality)
showFor redWines (fun wine -> wine.Density, wine.Quality)
showFor redWines (fun wine -> wine.``Fixed acidity``, wine.Quality)
showFor redWines (fun wine -> wine.``Free sulfur dioxide``, wine.Quality)
showFor redWines (fun wine -> wine.PH, wine.Quality)
showFor redWines (fun wine -> wine.``Residual sugar``, wine.Quality)
showFor redWines (fun wine -> wine.Sulphates, wine.Quality)
showFor redWines (fun wine -> wine.``Total sulfur dioxide``, wine.Quality)
showFor redWines (fun wine -> wine.``Volatile acidity``, wine.Quality)
showFor redWines (fun wine -> wine.Quality, wine.Quality)
