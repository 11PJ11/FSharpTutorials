...
for demoPhrase in demoPhrases2 do
    printfn "%s" demoPhrase.Question
    for answer in demoPhrase.Answers do
        printfn " >> %s" answer
...
type PsychoPhrasesProvider = JsonProvider< "[{\"pattern\":\"Something\",\"answers\":[\"Something else.\"]}]" >
...
// -------------------------------------------------
let dataPhrases = PsychoPhrasesProvider.Load("../data/phrases.json")

let phrases : Phrase list = 
    [ for phrase in dataPhrases do
          yield { InputPattern = 
                      { Contents = 
                            phrase.Pattern
                            |> cleanText
                  ...
                  AnswerPatterns = phrase.Answers |> Array.map parseText } ]
...
type ReflectionsProvider = JsonProvider< "../data/reflections.json" >

let reflections = ReflectionsProvider.GetSamples()

let reflectionDict = 
    [ for reflection in reflections do
          yield (reflection.Input, reflection.Output) ]
    |> dict

// 2. Reflect all the words in the text that can be reflected
// based on the dictionary
let reflect (words : string list) = 
    [ for word in words do
          if reflectionDict.ContainsKey(word) 
          then yield reflectionDict.[word]
          else yield word ]
