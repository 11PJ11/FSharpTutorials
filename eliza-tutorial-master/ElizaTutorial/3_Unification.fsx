...
// -------------------------------------------------
let rec demoCompare aList anotherList acc = 
    match aList, anotherList with
    | x :: xs, y :: ys when x = y -> 
...
// -------------------------------------------------
let rec unification : Pattern list -> Pattern list -> Pattern list -> Pattern list option = 
    fun pattern input acc -> 
        ...
        | [], [] -> 
            acc
            |> List.rev
            |> Some //  - if both are empty, we succeeded!

        | p :: ps, i :: iis when p = i -> unification ps iis acc
        | p :: ps, i :: iis when p = Wildcard -> 
            let matchEnds = unification ps (i :: iis) acc //     * recursively match rest of the pattern (without wildcard)
            match matchEnds with
            | None -> unification (p :: ps) (iis) (i :: acc)
            | Some _ -> 
                acc
                |> List.rev
                |> Some //       with the original input (and don't assing more words to wildcard)
        | _, _ -> None //  - otherwise we fail (because the pattern doesn't match input)
...
(unification (parseText "a *") (parseText "a b c") []) = Some [ Word "b"
                                                                Word "c" ]
(unification (parseText "* a *") (parseText "a a b c") []) = Some [ Word "a"
                                                                    Word "b"
                                                                    Word "c" ]
...
// -------------------------------------------------
let matchPattern (pattern : Sentence) (input : Sentence) = None // CHANGE ME
